...
public class VersionServiceImpl implements VersionService {
    ...
    public Snapshot createSnapshot(Snapshot snapshot) {
        ...
        snapshotRepository
                .findFirstByFileIdAndBranchOrderByCreatedAtDesc(
                        snapshot.getFileId(), snapshot.getBranch())
        ...
        branchRepository.findByProjectIdAndName(snapshot.getProjectId(), snapshot.getBranch())
                .ifPresent(branch -> {
                    branch.setHeadSnapshotId(saved.getSnapshotId());
    ...
    @Override
    public List<Snapshot> getSnapshotsByBranch(String fileId, String branch) {
        return snapshotRepository.findByBranchAndFileId(branch, fileId);
    }

    @Override
    public Snapshot getLatestSnapshot(String fileId, String branch) {
    ...
    public Snapshot restoreSnapshot(String snapshotId) {
        ...
        Snapshot restored = Snapshot.builder()
                ...
                .fileId(old.getFileId())
                .authorId(old.getAuthorId())
                .message("Restored from: " + old.getMessage())
                .content(old.getContent())
                .branch(old.getBranch())
                .build();
    ...
    public List<String> diffSnapshots(String snapshotId1, String snapshotId2) {
        ...
        return computeDiff(s1.getContent(), s2.getContent(),
                "snapshot/" + snapshotId1, "snapshot/" + snapshotId2);
    }

    /**
     * Simple line-by-line diff using pure Java — no external library needed.
     * Produces unified diff format: --- +++ @@ lines.
     */
    private List<String> computeDiff(String original, String revised,
            ...
            if (i < oldLines.length && j < newLines.length
                ...
                for (int di = 0; di <= 5; di++) {
                    for (int dj = 0; dj <= 5; dj++) {
                        if (i + di < oldLines.length && j + dj < newLines.length
                                && oldLines[i + di].equals(newLines[j + dj])) {
                            matchI = i + di;
                            matchJ = j + dj;
                            break outer;
                        }
    ...
    public Branch createBranch(String projectId, String name, String createdById) {
        ...
        return branchRepository.save(Branch.builder()
                .projectId(projectId).name(name).createdById(createdById).build());
    }
